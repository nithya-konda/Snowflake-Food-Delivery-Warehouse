...
def fetch_kpi_data():
    ...
    SELECT 
        ...
        max_order_value
    FROM food_delivery_db.enriched_sch.vw_yearly_revenue_kpis
    ORDER BY year;
    """
    return session.sql(query).collect()

# Function to fetch monthly KPI data
def fetch_monthly_kpi_data(year):
    ...
    FROM 
        food_delivery_db.enriched_sch.vw_monthly_revenue_kpis
    WHERE year = {year}
    ORDER BY month;
    """
    return session.sql(query).collect()
...
def fetch_unique_months(year):
    ...
    SELECT 
        DISTINCT MONTH
    FROM 
        food_delivery_db.enriched_sch.vw_monthly_revenue_by_restaurant
    WHERE YEAR = {year}
    ORDER BY MONTH;
    """
    return session.sql(query).collect()
...
if not previous_year_data.empty:
    ...
    revenue_diff = None
    orders_diff = None
    avg_rev_order_diff = None
    avg_rev_item_diff = None
    max_order_diff = None
...
with col1:
    ...
    st.metric(
        "Total Orders",
        f"{total_orders:,}",
        delta=f"{orders_diff:,}" if orders_diff is not None else None
    )
...
bar_chart = alt.Chart(month_df).mark_bar(
    color="#ff5200"
).encode(
...
trend_chart = alt.Chart(month_df).mark_line(
    color="#ff5200",
    point=alt.OverlayMarkDef(color="#ff5200")
).encode(
    x=alt.X('Month', sort='ascending', title='Month'),
    y=alt.Y(
...
if selected_year:
    ...
    if selected_month:
        ...
        top_restaurants = fetch_top_restaurants(
            selected_year,
            selected_month
        )
        ...
        if top_restaurants:
            ...
            styled_df = top_restaurants_df.style.apply(
                highlight_rows,
                axis=1
            )
