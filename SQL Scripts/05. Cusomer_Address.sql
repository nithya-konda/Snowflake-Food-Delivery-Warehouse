...
create or replace table raw_sch.customeraddress (
...
create or replace stream raw_sch.customeraddress_stm 
on table raw_sch.customeraddress
append_only = true
comment = 'This is the append-only stream object on customer address table that only gets delta data';


select * from raw_sch.customeraddress_stm;
copy into raw_sch.customeraddress (addressid, customerid, flatno, houseno, floor, building, 
...
from (
    ...
    from @raw_sch.csv_stg/initial/customer_address t
)
...
-- 2nd layer
CREATE OR REPLACE TABLE CURATED_SCH.CUSTOMER_ADDRESS (
...
create or replace stream CURATED_SCH.CUSTOMER_ADDRESS_STM
on table CURATED_SCH.CUSTOMER_ADDRESS
comment = 'This is the stream object on customer address entity to track insert, update, and delete changes';


MERGE INTO curated_sch.customer_address AS clean
USING (
    SELECT 
        ...
        _stg_file_md5,
        _copy_data_ts
    FROM raw_sch.customeraddress_stm 
) AS stage
...
CREATE OR REPLACE TABLE ENRICHED_SCH.CUSTOMER_ADDRESS_DIM (
...
-- select * from CURATED_SCH.CUSTOMER_ADDRESS_STM;
MERGE INTO 
    ENRICHED_SCH.CUSTOMER_ADDRESS_DIM AS target
USING 
    CURATED_SCH.CUSTOMER_ADDRESS_STM AS source
ON 
...
--
select * from raw_sch.customeraddress;
select * from CURATED_SCH.CUSTOMER_ADDRESS;
select * from ENRICHED_SCH.CUSTOMER_ADDRESS_DIM;


list @raw_sch.csv_stg/delta/customer_address;
copy into raw_sch.customeraddress (addressid, customerid, flatno, houseno, floor, building, 
                               landmark, locality,city,pincode, state, coordinates, primaryflag, addresstype, 
                               createddate, modifieddate, 
                               _stg_file_name, _stg_file_load_ts, _stg_file_md5, _copy_data_ts)
from (
    select 
        ...
        metadata$file_content_key as _stg_file_md5,
        current_timestamp as _copy_data_ts
    from @raw_sch.csv_stg/delta/customer_address_book/day-01-customer-address-book.csv t
)
file_format = (format_name = 'raw_sch.csv_file_format')
on_error = abort_statement;
