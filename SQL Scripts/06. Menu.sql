...
create or replace table raw_sch.menu (
...
create or replace stream raw_sch.menu_stm 
on table raw_sch.menu
append_only = true
comment = 'This is the append-only stream object on menu entity that only gets delta data';


list @raw_sch.csv_stg/initial/menu;

copy into raw_sch.menu (menuid, restaurantid, itemname, description, price, category, 
...
from (
    ...
    from @raw_sch.csv_stg/initial/menu t
)
...
CREATE OR REPLACE TABLE curated_sch.menu (
...
create or replace stream CURATED_SCH.menu_stm 
on table CURATED_SCH.menu
comment = 'This is the stream object on menu table table to track insert, update, and delete changes';


MERGE INTO curated_sch.menu AS target
USING (
    SELECT 
...
CREATE OR REPLACE TABLE enriched_sch.menu_dim (
...
MERGE INTO 
    enriched_sch.MENU_DIM AS target
USING 
    CURATED_SCH.MENU_STM AS source
ON 
...
-- Part -2
list @raw_sch.csv_stg/delta/menu;
copy into raw_sch.menu (menuid, restaurantid, itemname, description, price, category, 
                availability, itemtype, createddate, modifieddate,
                _stg_file_name, _stg_file_load_ts, _stg_file_md5, _copy_data_ts)
from (
    select 
        ...
        metadata$file_content_key as _stg_file_md5,
        current_timestamp as _copy_data_ts
    from @raw_sch.csv_stg/delta/menu/ t
)
file_format = (format_name = 'raw_sch.csv_file_format')
on_error = abort_statement;