...
create or replace table raw_sch.restaurant (
    restaurantid text,      
...
create or replace stream raw_sch.restaurant_stm 
on table raw_sch.restaurant
append_only = true
...
copy into raw_sch.restaurant (restaurantid, name, cuisinetype, pricing_for_2, restaurant_phone, 
...
from (
    select 
     ...
     from @raw_sch.csv_stg/initial/restaurant/restaurant-delhi+NCR.csv t
)
...
create or replace table curated_sch.restaurant (
...
create or replace stream curated_sch.restaurant_stm 
on table curated_sch.restaurant
...
insert into curated_sch.restaurant (
    restaurant_id,
    name,
    cuisine_type,
    pricing_for_two,
...
MERGE INTO curated_sch.restaurant AS target
USING (
    SELECT 
        try_cast(restaurantid AS number) AS restaurant_id,
        try_cast(name AS string) AS name,
...
CREATE OR REPLACE TABLE ENRICHED_SCH.RESTAURANT_DIM (
...
select count(*) from CURATED_SCH.RESTAURANT_STM;

-- merge statement
MERGE INTO 
    ENRICHED_SCH.RESTAURANT_DIM AS target
USING 
    CURATED_SCH.RESTAURANT_STM AS source
ON 
...
list @raw_sch.csv_stg/delta/restaurant/;
...
from (
    select 
        ...
        metadata$file_content_key as _stg_file_md5,
        current_timestamp() as _copy_data_ts
     from @raw_sch.csv_stg/delta/restaurant/day-01-insert-restaurant-delhi+NCR.csv t
)
file_format = (format_name = 'raw_sch.csv_file_format')
on_error = abort_statement;
...
from (
    select 
        ...
        metadata$file_content_key as _stg_file_md5,
        current_timestamp() as _copy_data_ts
     from @raw_sch.csv_stg/delta/restaurant/day-02-upsert-restaurant-delhi+NCR.csv t
)
file_format = (format_name = 'raw_sch.csv_file_format')
on_error = abort_statement;
