...
create or replace table raw_sch.deliveryagent (
...
create or replace stream raw_sch.deliveryagent_stm 
on table raw_sch.deliveryagent
append_only = true
comment = 'This is the append-only stream object on delivery agent table that only gets delta data';


copy into raw_sch.deliveryagent (deliveryagentid, name, phone, vehicletype, locationid, 
...
from (
    ...
    from @raw_sch.csv_stg/initial/delivery-agent t
)
file_format = (format_name = 'raw_sch.csv_file_format')
on_error = abort_statement;

select count(*) from raw_sch.deliveryagent;

select * from raw_sch.deliveryagent_stm;


CREATE OR REPLACE TABLE curated_sch.delivery_agent (
...
create or replace stream CURATED_SCH.delivery_agent_stm 
on table CURATED_SCH.delivery_agent
comment = 'This is the stream object on delivery agent table table to track insert, update, and delete changes';


MERGE INTO curated_sch.delivery_agent AS target
USING raw_sch.deliveryagent_stm AS source
...
select * from CURATED_SCH.delivery_agent_stm ;


CREATE OR REPLACE TABLE enriched_sch.delivery_agent_dim (
...
MERGE INTO enriched_sch.delivery_agent_dim AS target
USING CURATED_SCH.delivery_agent_stm AS source
ON 
...
from (
    select 
        ...
        metadata$file_content_key as _stg_file_md5,
        current_timestamp as _copy_data_ts
    from @raw_sch.csv_stg/delta/delivery_agent/ t
)
file_format = (format_name = 'raw_sch.csv_file_format')
on_error = abort_statement;