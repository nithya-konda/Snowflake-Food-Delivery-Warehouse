...
comment = 'This is the location raw table where data will be copied from internal stage using copy command. This is as-is data represetation from the source location. All the columns are text data type except the audit columns that are added for traceability.'
;

create or replace stream raw_sch.location_stm 
on table raw_sch.location
append_only = true
comment = 'this is the append-only stream object on location table that gets delta data based on changes';

select * from raw_sch.location;

copy into raw_sch.location (locationid, city, state, zipcode, activeflag, 
...
from (
    ...
    from @raw_sch.csv_stg/initial/location t
)
...
select * from raw_sch.location;
select * from raw_sch.location_stm;

use schema curated_sch;

-- Level 2
create or replace table curated_sch.restaurant_location (
...
comment = 'Location entity under curated schema with appropriate data type under curated schema layer, data is populated using merge statement from the raw layer location table. This table does not support SCD2';

create or replace stream curated_sch.restaurant_location_stm 
on table curated_sch.restaurant_location
comment = 'this is a standard stream object on the location table to track insert, update, and delete changes';


MERGE INTO curated_sch.restaurant_location AS target
USING (
    SELECT 
...
create or replace table enriched_sch.restaurant_location_dim (
...
MERGE INTO 
        ENRICHED_SCH.RESTAURANT_LOCATION_DIM AS target
    USING 
        CURATED_SCH.RESTAURANT_LOCATION_STM AS source
    ON 
...
LIST @raw_sch.csv_stg/delta/location
...
from (
    select 
        ...
        metadata$file_content_key as _stg_file_md5,
        current_timestamp as _copy_data_ts
    from @raw_sch.csv_stg/delta/location/delta-day01-2rows-add.csv t
)
file_format = (format_name = 'raw_sch.csv_file_format')
on_error = abort_statement;
