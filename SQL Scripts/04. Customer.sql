...
create or replace table raw_sch.customer (
...
create or replace stream raw_sch.customer_stm 
on table raw_sch.customer
append_only = true
...
copy into  raw_sch.customer (customerid, name, mobile, email, loginbyusing, gender, dob, anniversary, 
...
from (
    ...
    from @raw_sch.csv_stg/initial/customers/customers-initial.csv t
)
file_format = (format_name = 'raw_sch.csv_file_format')
on_error = abort_statement;

select * from raw_sch.customer limit 10;
select count(*) from raw_sch.customer; 
select count(*) from raw_sch.customer_stm; 


-- Part-2 Clean Layer
-- 

CREATE OR REPLACE TABLE CURATED_SCH.CUSTOMER (
...
-- Stream object to capture the changes. 
create or replace stream CURATED_SCH.customer_stm 
on table CURATED_SCH.customer
comment = 'This is the stream object on customer entity to track insert, update, and delete changes';


insert into curated_sch.customer (
    customer_id,
    name,
    mobile,
    email,
...
select 
    ...
    _stg_file_md5,
    _copy_data_ts
from raw_sch.customer;


MERGE INTO CURATED_SCH.CUSTOMER AS target
USING (
    SELECT 
...
-- create dim table 
CREATE OR REPLACE TABLE ENRICHED_SCH.CUSTOMER_DIM (
...
MERGE INTO 
    ENRICHED_SCH.CUSTOMER_DIM AS target
USING 
    CURATED_SCH.CUSTOMER_STM AS source
ON 
...
list @raw_sch.csv_stg/delta/customers/;
...
from (
    select 
        ...
        metadata$file_content_key as _stg_file_md5,
        current_timestamp as _copy_data_ts
    from @raw_sch.csv_stg/delta/customers/day-01-insert-customer.csv t
)
file_format = (format_name = 'raw_sch.csv_file_format')
on_error = abort_statement;
...
from (
    select 
        ...
        metadata$file_content_key as _stg_file_md5,
        current_timestamp as _copy_data_ts
    from @raw_sch.csv_stg/delta/customers/day-02-insert-update.csv t
)
file_format = (format_name = 'raw_sch.csv_file_format')
on_error = abort_statement;